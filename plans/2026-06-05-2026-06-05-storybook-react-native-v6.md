...
| Babel                     | `babel-preset-expo` + plugins (see Environment Variable Handling below)                          |

### Why v10 (not v8 or v9)

- npm registry: `@storybook/react-native@10.4.4` is the latest stable
- Peer dependencies: `storybook@>=10`, `react@*`, `react-native@>=0.72.0`
- Key v10 improvement: Safe top-level imports — when the Metro `withStorybook` wrapper has `enabled: false`, the `.rnstorybook` import is stubbed out to an empty module. No conditional imports needed, production bundles are zero-overhead.
- Uses `@storybook/react@^10.4.0` internally but all public API types (`Meta`, `StoryObj`, `Preview`) are re-exported from `@storybook/react-native`
...
- When `EXPO_PUBLIC_STORYBOOK_ENABLED` is not set to `'true'`, the Metro wrapper stubs out the `.rnstorybook` import, so production builds contain zero Storybook code

### Environment Variable Handling

This is critical and must be done correctly:

**`babel.config.js:23`** uses `transform-inline-environment-variables` with a specific `include` list:

```js
['transform-inline-environment-variables', {include: requiredEnvVars}];
```

where `requiredEnvVars = ['MAPBOX_ACCESS_TOKEN', 'COMAPEO_METRICS_URL', 'COMAPEO_METRICS_API_KEY']`.

These vars are asserted as required at the top of `babel.config.js:9-13`, then inlined into the bundle at build time.

**For Storybook, use `EXPO_PUBLIC_STORYBOOK_ENABLED`** (not `STORYBOOK_ENABLED`):

- Expo has a convention: any env var prefixed with `EXPO_PUBLIC_` is automatically available as `process.env.EXPO_PUBLIC_*` in the Metro bundle — no babel config changes needed.
- The v10 `@storybook/react-native` docs and migration guide use `process.env.EXPO_PUBLIC_STORYBOOK_ENABLED` for Expo projects.
- `STORYBOOK_ENABLED` (without the prefix) would NOT be available at runtime in the bundle unless added to babel's `include` list and the assert — but we don't want to make it required.

**Two places where the env var is used:**

1. **Metro config** (Node.js at build time): `enabled: process.env.EXPO_PUBLIC_STORYBOOK_ENABLED === 'true'` — controls import stubbing
2. **`App.tsx` conditional** (in the bundle at runtime): `process.env.EXPO_PUBLIC_STORYBOOK_ENABLED === 'true'` — controls which component renders

Setting `EXPO_PUBLIC_STORYBOOK_ENABLED=true` in the shell environment makes it available in both contexts.
...
- [ ] **1.1** Install dependencies as devDependencies:
  ...
  - `@react-native-async-storage/async-storage` (required by Storybook's `getStorybookUI` for persistent storage)
  - `storybook@^10.4.0` (peer dependency of `@storybook/react-native@10` — the npm registry lists this as required)

  The project already has these as regular dependencies (no install needed, but verify versions satisfy peer ranges):
  - `react-native-reanimated` (v3.19.5, peer: `>=2`)
  - `react-native-gesture-handler` (v2.31.0, peer: `>=2`)
  - `react-native-safe-area-context` (v5.7.0, peer: `*`)
...
- [ ] **1.2** Create `.rnstorybook/` directory with three files:
  ...
  **`.rnstorybook/main.ts`**:
  ...
  const main: StorybookConfig = {
    stories: ['../src/frontend/**/*.stories.?(ts|tsx|js|jsx)'],
    addons: [],
  };
...
- [ ] **1.4** Update `metro.config.js` to wrap the existing config with the **Metro-specific** `withStorybook`. This must be the Metro-specific import (`@storybook/react-native/metro/withStorybook`), NOT the bundler-agnostic one (`@storybook/react-native/withStorybook`). Apply it as the outermost wrapper on the final config object:
  ...
  // ... existing config setup (getSentryExpoConfig, SVG transformer, blockList, unstable_enablePackageExports) ...
  ...
  Note: The `withStorybook` wrapper must receive the FINAL config object. Import order: Sentry → SVG transformer → blockList → `withStorybook`.
...
- [ ] **1.5** Modify `src/frontend/App.tsx` to conditionally render Storybook instead of `AppNavigator`. Add the import at the top and modify line 249:
  ...
  The import path from `src/frontend/App.tsx` to `.rnstorybook/` is `../../.rnstorybook`. Verify this at implementation time — if Metro resolves it differently, adjust.

  **Important**: The `EXPO_PUBLIC_` prefix means this env var is available at runtime in the Expo bundle without modifying `babel.config.js`. When `EXPO_PUBLIC_STORYBOOK_ENABLED` is not set, Metro stubs the import and the conditional evaluates to `false`.

- [ ] **1.6** Add npm scripts to `package.json`:
  - `"storybook": "EXPO_PUBLIC_STORYBOOK_ENABLED=true npm run storybook-generate && EXPO_PUBLIC_STORYBOOK_ENABLED=true npm run prestart && EXPO_PUBLIC_STORYBOOK_ENABLED=true expo start"`
  - `"storybook:android": "npm run build:backend && EXPO_PUBLIC_STORYBOOK_ENABLED=true expo run:android"`

  The `storybook` script regenerates stories, builds translations/polyfills, then starts Expo. For subsequent runs (when no stories were added), you can just run `EXPO_PUBLIC_STORYBOOK_ENABLED=true npm start`.
...
- [ ] **2.2** Create `.rnstorybook/decorators/withNavigation.tsx` — wraps a single screen component in `NavigationContainer` + `NativeStack.Navigator` + `NativeStack.Screen`. Provides `useNavigation()`, `useRoute()`, `useFocusEffect()` context. Accepts initial route params via story parameters. This bypasses `RootStackNavigator` entirely (no auth checks, no device name checks, no `ActiveProjectProvider`). Use `@react-navigation/native-stack` (already a devDependency at v7.3.21) and `@react-navigation/native` (already a dependency at v7.2.2).
  ...
  **Important limitation**: `RootStackNavigator` (at `src/frontend/Navigation/Stack/index.tsx:54-126`) conditionally renders screens based on auth state, device name, and project ID. It also wraps app screens in `ActiveProjectProvider` (line 79-82) which calls `useSingleProject({projectId})` from `@comapeo/core-react`. The `withNavigation` decorator bypasses all of this, so screens that depend on `ActiveProjectProvider` will need the `fullApp` decorator instead.

  **Navigation nesting caveat**: `RootStackNavigator` also renders `PendingInvitesListener` and `PendingMapSharesListener` in the `layout` function (lines 64-76). Screens that expect these listeners to be active won't have them with `withNavigation`. Most screens should be fine without them, but if a screen crashes due to missing listeners, switch to the `fullApp` decorator.

- [ ] **2.3** Create `.rnstorybook/decorators/fullApp.tsx` — a passthrough decorator that renders `<Story />` directly. With the in-app integration strategy, all app contexts (ComapeoCore, Location, LocalDiscovery, Auth, ActiveProject, etc.) are already available. This decorator exists for documentation — it marks stories that depend on the full app context.
...
- [ ] **2.5** Register the `minimal` decorator globally in `.rnstorybook/preview.tsx`:
...
- [ ] **8.1** Verify production builds exclude Storybook — run `expo run:android` (without `EXPO_PUBLIC_STORYBOOK_ENABLED`) and confirm the app works normally with no Storybook code.
...
- [ ] Production builds (without `EXPO_PUBLIC_STORYBOOK_ENABLED`) do not include Storybook code
...
1. **`@storybook/react-native@10.4.4` compatibility with Expo 54 + RN 0.81 + React 19**
   - Risk: Storybook RN v10 lists `react-native@0.85.3` in its devDependencies, `react@19.2.3`, and `babel-preset-expo@^55.0.13`. The project uses RN 0.81.6, React 19.1.4, and `babel-preset-expo@54.0.10`. These are minor version differences. Peer dependencies specify `react-native@>=0.72.0` which is satisfied. Expo 54 compatibility is the biggest unknown.
   - Mitigation: Phase 1 task 1.8 is a hard gate. If incompatible, try: (a) check GitHub issues for Expo 54 + Storybook RN v10, (b) try `@storybook/react-native@9` as fallback, (c) contribute a compatibility fix.

2. **Metro config conflicts**
   - Risk: The Metro-specific `withStorybook` wrapper may conflict with the existing `getSentryExpoConfig`, SVG transformer, blockList patterns, or `unstable_enablePackageExports`.
   - Mitigation: Apply `withStorybook` as the outermost wrapper on the final config. Test in Phase 1. If conflicts arise, inspect what `withStorybook` modifies and apply those changes manually to the config object.

3. **React Compiler errors**
   - Risk: `app.json:15` has `"reactCompiler": true`. Storybook files (`.rnstorybook/` and `.stories.tsx` files) may trigger compiler errors because they use patterns the compiler doesn't support or because the Storybook library itself uses patterns the compiler rejects.
...
4. **Sentry noise in development**
   - Risk: Sentry initializes at `App.tsx:76-84` module level with `tracesSampleRate: 1.0` when `appMetricsOptIn` is true (which it is for non-production). Storybook stories will trigger Sentry events.
   - Mitigation: The `sentryEnvironment` is set to `'development'` via `appVariant.ts:9` (`.dev` applicationId suffix). Production is unaffected. Filter in Sentry dashboard if noisy. Not worth adding conditional logic to disable Sentry in Storybook mode.
...
6. **`withNavigation` decorator limitations — `ActiveProjectProvider` dependency**
   - Risk: Many app screens depend on `ActiveProjectProvider` (wrapped in `RootStackNavigator` layout at `Navigation/Stack/index.tsx:79-82`), which calls `useSingleProject({projectId})` from `@comapeo/core-react`. The `withNavigation` decorator bypasses the entire `RootStackNavigator`, so these screens will crash.
   - Mitigation: Use `fullApp` decorator for screens that need `ActiveProjectProvider`. Use `withNavigation` only for screens that don't use `@comapeo/core-react` hooks (settings screens, onboarding screens, auth screen).

7. **`babel.config.js` required env vars**
   - Risk: `babel.config.js:3-13` asserts that `MAPBOX_ACCESS_TOKEN`, `COMAPEO_METRICS_URL`, and `COMAPEO_METRICS_API_KEY` are set at build time. These are already required for all normal builds.
   - Mitigation: No action needed — these env vars are already required for all builds including Storybook mode.
...
9. **`PendingInvitesListener` and `PendingMapSharesListener` crash in `withNavigation` stories**
   - Risk: These listeners are rendered in `RootStackNavigator`'s `layout` function (lines 64-76). Screens may expect them to be mounted and call navigation functions they expose. Without them, screens that reference the listeners' injected navigation callbacks will crash.
   - Mitigation: For any screen that crashes due to missing listeners, switch to the `fullApp` decorator which renders inside the real `RootStackNavigator`.

10. **Storybook WebSocket companion port conflict**
    - Risk: The Storybook web companion (optional desktop UI) uses port 7007. If that port is in use, the companion won't start.
    - Mitigation: Not needed for initial setup. The on-device UI works standalone. Add web companion later if desired.

11. **`storybook.requires.ts` not regenerated after adding stories**
    - Risk: The `sb-rn-get-stories` script must run whenever stories are added, renamed, or removed. Forgetting to run it means new stories won't appear.
    - Mitigation: The `storybook` npm script (task 1.6) runs `storybook-generate` before starting. Document this requirement in the README (task 8.2).
