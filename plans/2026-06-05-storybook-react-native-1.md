...
- [ ] **1.8** Verify Storybook renders inside the app on a device/emulator with a single placeholder story

### Phase 2: Decorator Library (1.5 days)

- [ ] **2.1** Create `storybook/decorators/minimal.tsx` — wraps stories with `LocaleContext` + `IntlProvider`. Port directly from `tests/integration/helpers/react.tsx:52-62` (`createMinimalWrapper()`). This is sufficient for all 66 pure sharedComponents.
...
- [ ] **2.3** Create `storybook/decorators/withNavigation.tsx` — wraps a single screen in a `NavigationContainer` with a minimal stack navigator containing just that screen. Provides `useNavigation`, `useRoute`, `useFocusEffect` context. Use `@react-navigation/native-stack`.
- [ ] **2.4** Create `storybook/decorators/fullApp.tsx` — the full decorator using real backend. Port `createManager()` from `tests/integration/helpers/core.ts:31-55` and `setUpIPC()` from `tests/integration/helpers/core.ts:57-79`. Wrap with `createAppProvidersWrapper()` from `tests/integration/helpers/react.tsx:64-222`. Creates a real in-memory `MapeoManager`, sets up IPC, creates a test project, and seeds sample data. Add cleanup in `useEffect` return.
...
- [ ] **2.8** Create `storybook/utils/seedData.ts` — helper functions to seed the in-memory backend with test data: create observations, tracks, presets, members, etc. Uses the `MapeoProjectApi` from the real backend.

### Phase 3: Leaf Component Stories (1 day)

These 66 pure `sharedComponents` need only the `minimal` decorator (IntlProvider). No native dependencies.
...
### Phase 5: Screen Stories — Mocked Backend (2-3 days)

These screens use `@comapeo/core-react` hooks but can work with the `fullApp` decorator that provides a real in-memory backend. Each screen story needs: `fullApp` decorator + `withNavigation` decorator + seeded test data.
...
- [ ] The `fullApp` decorator creates a working in-memory backend and renders screen stories with seeded data
...
1. **`@storybook/react-native` v8 compatibility with Expo 54**
   - Risk: Storybook RN v8 may have unresolved issues with Expo SDK 54 (which is very recent).
   - Mitigation: Test the init immediately in Phase 1. If incompatible, check for canary releases or pin to a known-working combination. The `@storybook/react-native` project actively maintains Expo compatibility.

2. **Metro bundler conflicts**
   - Risk: Storybook's Metro configuration may conflict with the project's existing `metro.config.js` (which has custom SVG transformer and other settings).
...
3. **`@comapeo/core-react` provider initialization time**
   - Risk: The `fullApp` decorator creates a real `MapeoManager` with IPC setup. This takes ~1-2 seconds per story load, making story switching slow.
   - Mitigation: Cache the manager instance across stories (create once, reuse). Use the `withStores` decorator for static UI stories that don't need real data. Only use `fullApp` for stories that specifically test data-driven rendering.

4. **Memory pressure from in-memory backend**
   - Risk: Creating a new `MapeoManager` per story could leak memory on device, especially on lower-end Android devices.
   - Mitigation: Implement proper teardown in the decorator (call `stop()`, `closeMapeoClient()`, close `MessageChannel` ports). Reuse a single manager instance where possible. Add a "reset" button in Storybook that tears down and recreates the backend.
...
| ------------------------------------ | ------------------------------------------ | -------------- | ------------ |
| Phase 1: Infrastructure Setup        | Install, configure, toggle                 | 0.5 day        | 0.5 day      |
...
| Phase 5: Screen Stories              | ~80+ screen stories with fullApp decorator | 2-3 days       | 5.5-6.5 days |
| Phase 6: Navigation Flow Stories     | 4 multi-screen flows                       | 1 day          | 6.5-7.5 days |
| Phase 7: Documentation & Maintenance | Docs, CI, dev menu                         | 0.5 day        | 7-8 days     |

**Total: ~7-8 working days** for full coverage of all renderable UI elements.
